...
int main(int argc, char* argv[]) {
    ...
    // capture variable because not constexpr
    {
        ...
        std::function<int(std::string_view)> f = [INCREMENT](std::string_view s) {
            return s.size() + INCREMENT;
        };
    ...
    // capture the number of times the function is executed
    {
        ...
        int numberOfCalls = 0;
        std::function<int(std::string_view)> f = [&numberOfCalls](std::string_view s) {
            ++numberOfCalls;
    ...
    // pass non-static method of a class
    {
        ...
        std::function<int(std::string_view)> f = [&incrementer](std::string_view s) {

            return incrementer.incrementLengthWithLogging(s);
        };
